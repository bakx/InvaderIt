        ...
        loader.load((loader: any, resources: any) => {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                for (let key in resourceFile.data.frames) {
                    ...
                    let animationIndex = parseInt(key.slice(animationKey.length + 1, key.length));

                    // Validate index
                    if (Number.isNaN(animationIndex)) {
                        throw new Error(`Configuration issue. Animation index could not get stripped from file. Animation Key ${key}`);
                    }
